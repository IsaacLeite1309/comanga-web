...
// Guarda somente a página durante a sessão da aba, inclusive ao recarregar.
const rememberedPages = new Map<string, number>();
const storagePrefix = "comanga:catalog-page:";

export function readCatalogPage(key: string) {
  const remembered = rememberedPages.get(key);
  if (remembered) return remembered;
  try {
    const stored = Number(sessionStorage.getItem(storagePrefix + key));
...
export function rememberCatalogPage(key: string, page: number) {
  ...
  try {
    sessionStorage.setItem(storagePrefix + key, String(page));
  } catch {
    // Navegação continua em memória quando o navegador bloqueia armazenamento.
  }
}
