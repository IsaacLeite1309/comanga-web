...
describe("EditWork - paginação de Edições", () => {
  ...
  it("reconsulta a página atual após alterar a visibilidade", async () => {
    let reads = 0;
    arrangeEditions((page) => {
      reads += 1;
      return Promise.resolve({ data: editionsPage(page, 10) });
    });
    vi.mocked(api.patch).mockResolvedValue({ data: { edition: {
      ...editionsPage(2, 10).editions[0], visibility: "Público",
    } } });
    renderEditWork();
    await screen.findByText("Exibindo 8 de 10 Edições");
    fireEvent.click(screen.getByRole("button", { name: "Próxima página de Edições" }));
    await screen.findByText("Exibindo 2 de 10 Edições");
    fireEvent.click(screen.getByRole("button", { name: /alterar visibilidade da 2ª edição/i }));
    await waitFor(() => expect(reads).toBe(3));
    expect(await screen.findByText("2 / 2")).toBeInTheDocument();
    expect(vi.mocked(api.get).mock.lastCall).toEqual([
      "/admin/works/10/editions", { params: { order: "DESC", page: 2, limit: EDITIONS_PAGE_SIZE } },
    ]);
  });
  ...
  it("mostra o erro da lista de Edições mantendo o gerenciamento de Edições", async () => {
    ...
    expect(screen.getByRole("heading", { name: "Edições" })).toBeInTheDocument();
  });
});
